...
class SimpleStorageService:
    ...
    def read_object(object_name: str, decode: bool = True, make_readable: bool = False) -> Union[StringIO, str]:
        ...
        Args:
            ...
            decode (boo) : Whether to decode the object content as a string.
    ...
    def get_file_object(self, filename: str, bucket_name: str) -> Union[List[object], object]:
        """
        Retrieves the file Object(s) from the specified based on the filename.
        ...
        try:
            bucket = self.get_bucket(bucket_name)
            file_objs = [file_object for file_object in bucket.objects.filter(Prefix=filename)]
            func = lambda x: x[0] if len(x) == 1 else x
            logging.info("Exited the get_file_object method of SimpleStorageService class")
            return func(file_objs)
        except Exception as e:
            raise CustomException(e, sys) from e
    ...
    def read_csv(self, filename: str, bucket_name: str) -> DataFrame:
        """
        Reads a CSV fiel from the specified S3 bucket and converts it to a DataFrame.
