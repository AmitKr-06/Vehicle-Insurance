...
class ModelPusher:
    def __init__(self, model_evaluation_artifact: ModelEvaluationArtifact, model_pusher_config: ModelPusherConfig):
        """
        :param model_evalutation_artifact: Output refrence of data evaluation artifact stage
    ...
    def initiate_model_pusher(self) -> ModelPusherArtifact:
        ...
        try:
            ...
            self.model_pusher_artifact = ModelPusherArtifact(bucket_name = self.model_pusher_config.bucket_name, s3_model_path = self.model_pusher_config.s3_model_key_path)

            logging.info("Uploading artifacts folder to S3 bucket")
            logging.info(f"Model pusher artifact: [{self.model_pusher_artifact}]")
            logging.info("Exitedd initiate_model_pusher method of ModelTrainer class")
